...
public class BlepPeripheral{
	...
	public int init(PApplet parent, Blepdroid _blepdroid){
		...
		if(mManager == null)
		{
			mManager = (BluetoothManager)parent.getContext().getSystemService(Context.BLUETOOTH_SERVICE);
			...
			if(false == parent.getContext().getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE))
				return -2;          
		}      
		...
		if(mGattServer== null)
		{
			mGattServer = mManager.openGattServer(parent.getContext(), mGattServerCallback);
	...
	= new BluetoothGattServerCallback(){
		...
		public void onConnectionStateChange(BluetoothDevice device, int status, int newState){
			...
			if(mConnectionCallback != null && BluetoothGatt.GATT_SUCCESS == status)
	...
	{  
		...
		BluetoothGattCharacteristic softwareVerCharacteristic = 
				new BluetoothGattCharacteristic(UUID.fromString(SOFTWARE_REVISION_STRING), BluetoothGattCharacteristic.PROPERTY_READ, BluetoothGattCharacteristic.PERMISSION_READ);
		BluetoothGattService deviceInfoService = new BluetoothGattService(UUID.fromString(SERVICE_DEVICE_INFORMATION), BluetoothGattService.SERVICE_TYPE_PRIMARY);
